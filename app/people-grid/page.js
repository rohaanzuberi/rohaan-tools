...
function ShareBar() {
  ...
  const handleShare = (social) => {
    if (social.name === "Copy link") {
      navigator.clipboard.writeText(TOOL_URL).then(() => {
        setCopied(true);
  ...
  return (
    <div style={{
      ...
      <div style={{ textAlign: "center", marginBottom: "20px" }}>
        ...
        <p style={{ color: "rgba(28,28,28,0.45)", fontSize: "13px", lineHeight: "1.65", margin: 0 }}>
          Someone in your life needs this moment. Pass it on.
        </p>
      </div>
...
function PersonCard({ person, onUpdate, onRemove, index }) {
  ...
  return (
    <div style={{
      background: "#faf8f5", border: "1px solid rgba(44,44,44,0.1)",
